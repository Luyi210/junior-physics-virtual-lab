...
function authenticate(request, db, config) {
...
export function createApiHandler({ db, config }) {
  return async function handle(request, response) {
    ...
    try {
      ...
      if (request.method === "GET" && path === "/api/health") {
        return sendJson(response, 200, { status: "ok", service: "physics-lab-api", version: "0.1.0", compatibility: API_COMPATIBILITY, capabilities: ["managed-teacher-accounts", "managed-student-accounts", "class-membership", "teaching-tasks", "experiment-evidence"], database: "sqlite", time: new Date().toISOString() }, id, headers);
      }
      ...
      if (request.method === "POST" && path === "/api/auth/login") {
        ...
        const row = findUserForLogin(db, email);
      ...
      if (request.method === "GET" && path === "/api/users") {
        ...
        return sendJson(response, 200, listUsers(db, user, user.role === "teacher" ? "student" : role), id, headers);
      }
      if (request.method === "POST" && path === "/api/users") {
        ...
        return sendJson(response, 201, createManagedAccount(db, user, { email, password, name: requiredString(body, "name", { max: 80 }), role }), id, headers);
      }
      ...
      if (request.method === "PATCH" && params) {
        requireRole(user, "teacher", "admin");
        const body = await readJson(request, config.bodyLimitBytes);
        return sendJson(response, 200, updateManagedAccountStatus(db, user, params.userId, requiredEnum(body, "status", ["active", "disabled"])), id, headers);
      }
      ...
      if (request.method === "PATCH" && params) {
        ...
        return sendJson(response, 200, resetManagedAccountPassword(db, user, params.userId, password), id, headers);
      }

      if (request.method === "GET" && path === "/api/classes") return sendJson(response, 200, listClasses(db, user), id, headers);
      if (request.method === "POST" && path === "/api/classes") {
        requireRole(user, "teacher", "admin");
        const body = await readJson(request, config.bodyLimitBytes);
        return sendJson(response, 201, createClass(db, user, { name: requiredString(body, "name", { max: 80 }), grade: requiredString(body, "grade", { max: 30 }) }), id, headers);
      }

      params = match(path, "/api/classes/:classId");
      if (request.method === "GET" && params) {
        requireRole(user, "teacher", "admin");
        return sendJson(response, 200, getClass(db, params.classId, user.schoolId), id, headers);
      }
      ...
      if (request.method === "POST" && params) {
        requireRole(user, "teacher", "admin");
        const body = await readJson(request, config.bodyLimitBytes);
        return sendJson(response, 201, addClassMember(db, params.classId, user.schoolId, requiredString(body, "email", { max: 180 })), id, headers);
      }

      params = match(path, "/api/classes/:classId/members/:userId");
      if (request.method === "DELETE" && params) {
        requireRole(user, "teacher", "admin");
        return sendJson(response, 200, removeClassMember(db, params.classId, user.schoolId, params.userId), id, headers);
      }

      params = match(path, "/api/classes/:classId/live");
      if (request.method === "GET" && params) {
        requireRole(user, "teacher", "admin");
        return sendJson(response, 200, getLiveClass(db, user, params.classId), id, headers);
      }
      ...
      if (request.method === "POST" && params) {
        requireRole(user, "teacher", "admin");
        const body = await readJson(request, config.bodyLimitBytes);
        return sendJson(response, 201, addClassPrompt(db, user, params.classId, requiredString(body, "text", { max: 600 })), id, headers);
      }

      params = match(path, "/api/classes/:classId/report");
      if (request.method === "GET" && params) {
        requireRole(user, "teacher", "admin");
        return sendJson(response, 200, getClassReport(db, user, params.classId), id, headers);
      }

      if (request.method === "GET" && path === "/api/lessons") {
        requireRole(user, "teacher", "admin");
        return sendJson(response, 200, listLessons(db, user), id, headers);
      }
      if (request.method === "POST" && path === "/api/lessons") {
        requireRole(user, "teacher", "admin");
        const body = await readJson(request, config.bodyLimitBytes);
        return sendJson(response, 201, createLesson(db, user, {
      ...
      if (request.method === "GET" && path === "/api/tasks") return sendJson(response, 200, listTasks(db, user, url.searchParams.get("classId")), id, headers);
      if (request.method === "POST" && path === "/api/tasks") {
        requireRole(user, "teacher", "admin");
        const body = await readJson(request, config.bodyLimitBytes);
        return sendJson(response, 201, createTask(db, user, {
      ...
      if (request.method === "PATCH" && params) {
        requireRole(user, "teacher", "admin");
        const body = await readJson(request, config.bodyLimitBytes);
        return sendJson(response, 200, updateTaskStatus(db, user, params.taskId, requiredEnum(body, "status", ["draft", "published", "closed", "archived"])), id, headers);
      }

      params = match(path, "/api/tasks/:taskId/lesson-context");
      if (request.method === "GET" && params) {
        requireRole(user, "student");
        return sendJson(response, 200, getStudentTaskLesson(db, user, params.taskId), id, headers);
      }

      if (request.method === "POST" && path === "/api/sessions") {
        requireRole(user, "student");
        const body = await readJson(request, config.bodyLimitBytes);
        return sendJson(response, 201, createSession(db, user, { taskId: requiredString(body, "taskId") }), id, headers);
      }

      if (request.method === "GET" && path === "/api/sessions") {
        requireRole(user, "student");
        return sendJson(response, 200, listStudentSessions(db, user), id, headers);
      }
      ...
      if (request.method === "POST" && params) {
        requireRole(user, "student");
        const body = await readJson(request, config.bodyLimitBytes);
        return sendJson(response, 202, appendEvents(db, user, params.sessionId, validateEvents(body)), id, headers);
      }
      ...
      if (request.method === "POST" && params) {
        requireRole(user, "student");
        const body = await readJson(request, config.bodyLimitBytes);
        return sendJson(response, 201, addObservation(db, user, params.sessionId, requiredString(body, "text", { max: 2000 })), id, headers);
      }

      params = match(path, "/api/sessions/:sessionId/complete");
      if (request.method === "PATCH" && params) {
        requireRole(user, "student");
        return sendJson(response, 200, completeSession(db, user, params.sessionId), id, headers);
      }
