...
import { Activity, ArrowRight, BookMarked, Check, ChevronDown, ChevronLeft, ChevronRight, CircleCheck, GraduationCap, Lightbulb, Maximize2, MessageCircle, MousePointerClick, MoveHorizontal, PauseCircle, PlayCircle, RefreshCw, RotateCcw, Save, SendHorizontal, ShieldCheck, Sparkles, UserRound, Volume2, VolumeX, X } from "lucide-react";
import { Link, useLocation } from "react-router-dom";
import { diagnoseHarnessExperiment, getHarnessAdaptiveQuestions, getHarnessLearnerModel, getHarnessModuleGuide, getHarnessQuickQuestions } from "@physics-lab/harness";
...
type PanelView = "dialogue" | "tutorial";
...
function selectWarmMaleChineseVoice(voices: SpeechSynthesisVoice[]) {
  ...
  const preferredNames: Array<[RegExp, number]> = [
    [/Yunxi|Yunjian|Yunyang|Yunfeng|Yunhao|Kangkang|云希|云健|云扬/i, 140],
    [/Reed/i, 130],
    [/Eddy/i, 120],
    [/Rocko/i, 110],
    [/Grandpa|Li[- ]?mu|male|男/i, 100]
  ];
  const feminineNames = /Ting|Meijia|Sinji|Flo|Sandy|Shelley|Grandma|Xiaoxiao|female|女/i;
  return [...candidates].sort((left, right) => {
    const score = (voice: SpeechSynthesisVoice) => {
      ...
      return nameScore + languageScore + (voice.localService ? 4 : 0) - (feminineNames.test(voice.name) ? 80 : 0);
    };
...
function HarnessPanel() {
  ...
  const diagnosis = session && activeModule ? diagnoseHarnessExperiment(session, activeModule) : undefined;
  const learnerModel = session && activeModule ? getHarnessLearnerModel(session, activeModule) : undefined;
  const evidencedDimensions = learnerModel?.dimensions.filter((dimension) => dimension.score >= 50).length ?? 0;
  const quickQuestions = session && activeModule
    ? getHarnessAdaptiveQuestions(session, activeModule)
    : activeArea && activeModule ? getHarnessQuickQuestions(activeModule, activeArea) : [];
  ...
  return (
    <>
      <button className="harness-dock-trigger harness-dialogue-trigger cat-dock-trigger" onClick={() => setPanelOpen(true)} aria-label="打开橘猫实验员">
      ...
      {panelOpen && (
        <aside className={`harness-panel harness-dialogue-panel cat-partner-panel ${panelView === "tutorial" && tutorialCompact ? `tutorial-compact mini-side-${tutorialMiniSide}` : ""}`} role="dialog" aria-modal="true" aria-label="橘猫实验员交流与动态教程">
          ...
          <div className="harness-assurance harness-dialogue-assurance">
            <ShieldCheck size={16} />
            <span><strong>本地规则对话＋可操作动态教程</strong><small>不联网、不调用 LLM，教程只引导，不替你完成全部实验</small></span>
          </div>

          <nav className="cat-panel-tabs" aria-label="橘猫实验员功能">
            <button className={panelView === "dialogue" ? "active" : ""} onClick={() => { setTutorialCompact(false); setPanelView("dialogue"); }}><MessageCircle size={16} /><span>问问橘猫<small>交流与记录</small></span></button>
            <button className={panelView === "tutorial" ? "active" : ""} onClick={() => setPanelView("tutorial")}><GraduationCap size={17} /><span>动态教程<small>边看边操作</small></span></button>
          </nav>
          ...
          {loading || !session || !activeArea || !activeModule ? (
            ...
            <div className="harness-panel-body harness-dialogue-body">
              ...
              {diagnosis && <section className={`harness-inquiry-state stage-${diagnosis.stage}`} aria-label="光光的实验状态诊断">
              ...
              {learnerModel && <details className={`harness-learner-model level-${learnerModel.level}`}>
                ...
                <footer><p><Sparkles size={13} />这些状态只来自当前实验的操作和记录，不评价学生能力。{learnerModel.strength}；{learnerModel.memoryLine}。</p><aside><strong>下一项小挑战</strong><span>{learnerModel.nextChallenge}</span><button type="button" onClick={(event) => { event.preventDefault(); send("根据我的记录给我一个小挑战"); }}>让光光出题 <ArrowRight size={13} /></button></aside></footer>
              </details>}

              <section className="harness-hint-ladder" aria-label="光光三级提示">
                <header><span><Lightbulb size={14} />卡住了吗？按需要逐级打开提示</span><small>提示逐渐具体，但不会直接显示答案</small></header>
                <div><button type="button" onClick={() => send("给我一级提示")}><b>01</b><span><strong>观察方向</strong><small>先看什么</small></span></button><button type="button" onClick={() => send("给我二级提示")}><b>02</b><span><strong>操作脚手架</strong><small>怎样比较</small></span></button><button type="button" onClick={() => send("给我三级提示")}><b>03</b><span><strong>证据句式</strong><small>如何表达</small></span></button></div>
              </section>

              <Link className="harness-notebook-link" to="/student/notebook" onClick={() => setPanelOpen(false)}><BookMarked size={18} /><span><small>实验结束后</small><strong>把本次操作和发现整理进实验记录本</strong></span><ArrowRight size={15} /></Link>
                ...
                <div className="harness-chat-stream">
                  {dialogue.length === 0 && (
                    <article className="harness-message assistant welcome">
                      ...
                      <div><small>橘猫实验员 · 光光</small><p>喵，我已经连接到当前实验。你可以问我下一步观察什么、为什么会出现这个现象，或者切换到“动态教程”跟我一起做。</p><em>回答来自本地物理规则，不是大语言模型。</em></div>
                    </article>
                  )}
                  {dialogue.map((message) => (
                    <article className={`harness-message ${message.role}`} key={message.id}>
                      ...
                      <div><small>{message.role === "assistant" ? "橘猫实验员 · 光光" : "我的问题"}</small><p>{message.text}</p>{message.intent && <em>{message.intent.toUpperCase()} · RULE RESPONSE</em>}</div>
                    </article>
                  ))}
          ...
          <footer><span>CAT HARNESS · LOCAL TUTOR v0.4</span><button onClick={() => setPanelOpen(false)}>回到实验 <ArrowRight size={14} /></button></footer>
        </aside>
      )}
    </>
  );
...
function TutorialExperience({ activeArea, activeModule, compact, side, onCompactChange, onSideChange }: { activeArea: HarnessArea; activeModule: string; compact: boolean; side: "left" | "right"; onCompactChange: (compact: boolean) => void; onSideChange: (side: "left" | "right") => void }) {
  ...
  const interactionEvents = tutorialSession?.events.slice(interactionBaseline).filter((event) => (step?.acceptedEvents ?? []).includes(event.type)) ?? [];
  const latestInteraction = interactionEvents.at(-1);
  const genericInteractionSatisfied = !step?.requiresInteraction || interactionEvents.length > 0;
  const satisfied = activeModule === "dispersion" ? requirementMet(step?.requirement, snapshot) : genericInteractionSatisfied;
  const requiresAction = Boolean(step?.requirement || step?.requiresInteraction);
  ...
  useEffect(() => {
    ...
    const loadVoices = () => setNarratorVoice(selectWarmMaleChineseVoice(window.speechSynthesis.getVoices()));
    loadVoices();
  ...
  useEffect(() => {
    if (!started || !step?.requiresInteraction) return;
    setInteractionGate({ stepId: step.id, baseline: tutorialSession?.events.length ?? 0 });
  }, [started, step?.id, step?.requiresInteraction]);
  ...
  useEffect(() => {
    ...
    if (activeModule === "dispersion") {
      dispatchTutorialCommand({ type: "widget_highlight", module: "dispersion", target: step.target as TutorialTarget });
  ...
  }, [activeModule, available, narratorVoice, runVersion, started, step, voiceEnabled]);
  ...
  if (!started) {
    return <div className="harness-panel-body cat-tutorial-cover">
      ...
      <span>DYNAMIC LAB STORY · {activeModule === "dispersion" ? "约 4 分钟" : guidedProfile.duration}</span>
      <h2>{activeModule === "dispersion" ? "跟着橘猫，亲手发现白光里的颜色" : guidedProfile.title}</h2>
      <p>{activeModule === "dispersion" ? "这不是一段只能观看的视频。光光会边讲边指出装置，在关键步骤停下来，等你亲手打开光源、旋转三棱镜和移动光屏。" : guidedProfile.summary}</p>
      <ul>{(activeModule === "dispersion" ? ["9 个动态镜头", "3 次学生操作", "白光、单色光与 RGB 对照"] : guidedProfile.highlights).map((item) => <li key={item}><CircleCheck size={15} />{item}</li>)}</ul>
      <button onClick={begin}><PlayCircle size={19} />开始动态教程</button>
      <small>随时可以退出，原来的自由探索不会被关闭。</small>
    </div>;
  }
  ...
  if (compact) {
    return <div className="harness-panel-body cat-tutorial-mini-player">
      <TutorialSpotlight target={step.target} label={step.eyebrow} />
      ...
      <button className="cat-mini-copy" onClick={() => onCompactChange(false)} aria-label="展开完整讲解面板">
        <span>{step.eyebrow}</span><strong>{step.title}</strong><p>{requiresAction && !satisfied ? step.hint : step.narration}</p>
      </button>
  ...
  return <div className="harness-panel-body cat-tutorial-player">
    ...
    <section className="cat-tutorial-stage">
      <div className="cat-tutorial-avatar"><OrangeCatAvatar mood={catMood} /><i className={speaking ? "is-speaking" : ""}>{speaking ? "沉稳男声讲解中" : requiresAction && !satisfied ? "等你操作" : requiresAction ? "操作已记录" : "观察中"}</i></div>
      <div className="cat-speech-card" aria-live="polite"><span>{step.eyebrow}</span><h2>{step.title}</h2><p>{step.narration}</p></div>
    </section>
    ...
    {finished && <section className="cat-tutorial-finish" aria-label="动态教程完成">
      <CircleCheck size={23} />
      <div><small>TUTORIAL COMPLETE / 教程完成</small><strong>讲解结束，实验仍然保持开放。</strong><p>现在可以带着刚才的预测和证据继续自由改变条件，或者重新播放整个教程。</p></div>
      <button onClick={stop}>回到自由实验</button><button className="restart" onClick={begin}><RotateCcw size={14} />重新播放</button>
    </section>}
    ...
    <div className="cat-tutorial-tools">
      ...
      <button onClick={toggleVoice}>{voiceEnabled ? <Volume2 size={15} /> : <VolumeX size={15} />}{voiceEnabled ? "关闭男声" : "开启男声"}</button>
