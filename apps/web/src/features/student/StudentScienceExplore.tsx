...
const inquiryFormulas: Record<CoreLabKey | ConceptLabKey, string> = {
  ...
  "mechanics-speed": "v = s / t", "mechanics-friction": "匀速时 f = F拉", "mechanics-lever": "F₁l₁ = F₂l₂", "mechanics-pressure": "p = F / S", "mechanics-buoyancy": "ρ物 < ρ液 → 上浮",
...
function SoundLab() {
  ...
  const points = Array.from({ length: 101 }, (_, i) => { const x = i * 7.8; const y = 120 + Math.sin(i / 100 * Math.PI * 2 * (frequency / 55) + phase * Math.PI * 2) * amplitude; return `${x},${y}`; }).join(" ");
...
function CircuitLab() {
  ...
  return <LabFrame field="circuit" experiment="circuit-basic" eyebrow="CIRCUIT WIRING BENCH / 交互接线台" title="亲手接通电路，再让故障告诉你串联与并联的区别" description="选择任务后逐根连接接线柱。电路完整之前不能通电；通电后可以旋松任意灯泡，观察电流路径和另一只灯泡的变化。灯泡按定值电阻的等效模型计算。" running={closed} onToggle={toggleCircuit} playLabel="合上总开关" runningLabel="断开总开关" actionDisabled={!fullyWired} disabledLabel={`还差 ${plans.length - connectedWires.length} 根导线`}>
...
function ThermalLab() {
  ...
  useEffect(() => { if (!running) return; const timer = window.setInterval(() => setTime((value) => Math.min(240, value + 1.2)), 50); return () => clearInterval(timer); }, [running]);
  useEffect(() => { if (time >= 240) setRunning(false); }, [time]);
  ...
  return <LabFrame field="thermal" experiment="thermal-boiling" eyebrow="BOILING / 温度-时间图像" title="持续加热，温度曲线会怎样生长？" description="设水的初温为 25℃，以标准大气压下沸点 100℃、模拟加热效率 η=0.85 为条件。模型按 ηPt=cmΔT 计算升温，达到沸点后继续吸热但温度保持在沸点附近；未计入容器吸热和水分蒸发造成的质量变化。" running={running} onToggle={toggleHeat}>
    <div className="thermal-sim"><div className={`beaker-water ${temperature >= 99.8 ? "boiling" : "warming"}`} style={{ "--water-temp": `${temperature}%` } as React.CSSProperties}><div className="water-level" style={{ height: `${35 + mass / 8}%` }} />{hasHeated && Array.from({ length: temperature >= 99.8 ? 15 : 6 }, (_, i) => <i style={{ animationDelay: `${-i * .18}s`, left: `${12 + (i * 23) % 76}%` }} key={i} />)}</div><div className={`flame ${running ? "on" : ""}`}><i /><i /><i /></div><div className="thermometer"><i style={{ height: `${temperature * .75}%` }} /><span>{hasHeated ? `${temperature.toFixed(1)}℃` : "待测"}</span></div><svg viewBox="0 0 540 220"><line x1="35" y1="20" x2="35" y2="195"/><line x1="35" y1="195" x2="520" y2="195"/>{hasHeated && <><line className="boil-line" x1="35" y1="50" x2="520" y2="50"/><polyline points={graphPoints}/><text x="42" y="44">100℃</text></>}<text x="442" y="212">时间 / s</text></svg>{!hasHeated && <InteractionCue text="启动加热，生成你的第一段曲线" />}</div>
    <div className="science-controls four"><ScienceRange label="加热器功率" value={power} min={400} max={1200} step={50} unit="W" onChange={setPower} /><ScienceRange label="水的质量" value={mass} min={100} max={350} step={10} unit="g" onChange={setMass} /><ResultCell label="模拟时间" value={hasHeated ? `${time.toFixed(0)} s` : "等待加热"} pending={!hasHeated} /><button className="science-reset" onClick={() => { setRunning(false); setTime(0); setHasHeated(false); }}><RotateCcw size={15} />清空并重新测量</button></div>
  </LabFrame>;
}

function DensityLab() {
  const recordHarness = useHarnessStore((state) => state.record);
  const samples = { aluminum: { name: "铝块", density: 2.7, volume: 20, color: "#9ba8ad" }, iron: { name: "铁块", density: 7.8, volume: 12, color: "#485760" }, stone: { name: "石块", density: 2.5, volume: 25, color: "#68747b" } };
  ...
  const finalVolume = sample ? initialVolume + sample.volume : initialVolume;
  const measuredDensity = sample ? calculateDensity(mass, sample.volume) : 0;
  return <LabFrame field="measurement" experiment="measurement-density" eyebrow="DENSITY / 排水法测体积" title="不规则物体的体积，能用水“看见”吗？" description="先用天平测质量，再读取量筒初始体积 V₁；让不吸水、不溶于水且能完全浸没的样品没入水中，读取 V₂，物体体积为 V₂−V₁。" running={immersed} onToggle={() => { if (sample) setImmersed(!immersed); }} playLabel="完全浸没样品" runningLabel="从量筒取出样品" actionDisabled={!sample}>
    <div className="density-sim"><div className="balance"><div className="balance-beam"><i className="pan left">{sample && <span className="sample-block" style={{ background: sample.color }} />}</i><i className="pan right"><b>{sample ? `${mass.toFixed(1)}g` : "待称量"}</b></i></div><div className="balance-base"><Scale size={24} /><span>{sample ? `质量 ${mass.toFixed(1)} g` : "请先选择样品"}</span></div></div><div className="cylinder"><div className="water" style={{ height: `${42 + (immersed && sample ? sample.volume : 0) * 1.25}%` }} />{sample && <span className={`drop-sample ${immersed ? "immersed" : ""}`} style={{ background: sample.color }} />}{Array.from({ length: 7 }, (_, i) => <i style={{ bottom: `${12 + i * 10}%` }} key={i}>{40 + i * 10}</i>)}<b className="cylinder-live-reading">{immersed ? `V₂=${finalVolume} mL` : `V₁=${initialVolume} mL`}</b></div><div className={`density-result ${immersed ? "" : "awaiting-reading"}`}><small>{immersed ? "ρ = m / (V₂−V₁)" : "测量尚未完成"}</small><strong>{immersed ? `${measuredDensity.toFixed(2)} g/cm³` : "— —"}</strong><span>{sample ? immersed ? sample.name : "把样品完全浸没" : "从下方选择材料"}</span></div>{!sample && <InteractionCue text="选择一个可完全浸没的样品" />}</div>
    <div className="sample-tabs">{(Object.keys(samples) as Array<keyof typeof samples>).map((key) => <button className={sampleKey === key ? "active" : ""} onClick={() => { setSampleKey(key); setImmersed(false); recordHarness("configuration.changed", { control: "待测材料", value: samples[key].name }); }} key={key}>{samples[key].name}</button>)}</div><div className="science-controls four"><ResultCell label="天平测得质量" value={sample ? `${mass.toFixed(1)} g` : "等待选择样品"} pending={!sample} /><ResultCell label="量筒初、末示数" value={sample && immersed ? `${initialVolume} → ${finalVolume} mL` : `V₁ = ${initialVolume} mL`} pending={!immersed} /><ResultCell label="物体体积 V₂−V₁" value={sample && immersed ? `${sample.volume} cm³` : "等待完全浸没"} pending={!immersed} /><ResultCell label="计算密度" value={sample && immersed ? `${measuredDensity.toFixed(2)} g/cm³` : "完成测量后显示"} pending={!immersed} /></div>
  </LabFrame>;
}

function LabFrame({ field, experiment, eyebrow, title, description, running, onToggle, playLabel = "启动动态", runningLabel = "暂停", actionDisabled = false, disabledLabel = "先选择样品", children }: { field: FieldKey; experiment: CoreLabKey; eyebrow: string; title: string; description: string; running: boolean; onToggle: () => void; playLabel?: string; runningLabel?: string; actionDisabled?: boolean; disabledLabel?: string; children: React.ReactNode }) {
  const recordHarness = useHarnessStore((state) => state.record);
  const toggle = () => { onToggle(); recordHarness("simulation.toggled", { running: !running, action: playLabel }); };
  return <div className="science-lab"><header><div><span>{eyebrow}</span><h1>{title}</h1><p>{description}</p></div><PhysicsFieldMotif field={field} className="science-lab-physics" /><div className="science-lab-header-actions"><button className={running ? "running" : ""} onClick={toggle} disabled={actionDisabled}>{running ? <Pause size={16} /> : <Play size={16} />}{actionDisabled ? disabledLabel : running ? runningLabel : playLabel}</button><ExperimentFullscreenButton targetSelector=".science-lab" experiment={experiment} className="science-fullscreen-button" label="全屏工作台" hint="保留控制与读数" /></div></header>{children}</div>;
}
